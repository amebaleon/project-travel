...
- [ ] 2.1. **`db.py`**: SQLAlchemy와 `.env` 파일을 이용한 데이터베이스 연결 설정
- [ ] 2.2. **`db.py`**: 데이터베이스 세션 관리를 위한 `get_db` 함수 구현
- [ ] 2.3. **`db.py`**: `openapi.py`의 ORM 모델을 기반으로 DB 테이블을 생성하는 `create_tables` 함수 구현
- [ ] 2.4. **`db.py`**: Tour API로부터 관광 데이터를 가져오는 외부 API 연동 함수 구현
- [ ] 2.5. **`db.py`**: `APScheduler`를 사용하여 매주 Tour API 데이터를 DB에 저장하는 스케줄링 작업 설정
- [ ] 2.6. **`db.py`**: `tourist_info` 테이블에서 조건에 맞는 관광 정보를 조회하는 함수 구현
- [ ] 2.7. **`db.py`**: AI 상호작용 로그를 `ai_log` 테이블에 저장하는 함수 구현
